...
class aparat_com extends DownloadClass
{
    ...
    {
        if (!preg_match('@aparat\.com/v/(\w+)@i', $link, $vid)) {
        ...
        if (!preg_match('@<title>(?>(.*?)</title>)@is', $page, $title)) {
            html_error('Error: Video title not found.');
        }
        if (!preg_match_all('@https?://(?:[\w-]+\.)*aparat\.com/aparat-video/\w+(?:-(\d+p))?__\w+\.mp4@i', $page, $DL)) {
