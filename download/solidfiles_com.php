...
class solidfiles_com extends DownloadClass {
	...
	public function Download($link) {
		if(!preg_match('/solidfiles\.com\/v\/(\w+)/', $link, $link)) html_error('Invalid download link');
		$link = $link[1];
        ...
        $csrf = curl_exec($ch);
        if(!preg_match('/\'csrfmiddlewaretoken\' value=\'(.*?)\'/', $csrf, $csrf)) html_error('File not found.');
        $csrf = $csrf[1];
        curl_setopt($ch, CURLOPT_URL, 'http://www.solidfiles.com/v/'.$link.'/dl');
        curl_setopt($ch, CURLOPT_POSTFIELDS, array('csrfmiddlewaretoken' => $csrf, 'referrer' => ''));
        $dl = curl_exec($ch);
        curl_close($ch);
