...
class google_com extends DownloadClass
{
    ...
    private function checkBug78902($link = '')
    {
        ...
        if (substr(PHP_OS, 0, 3) == 'WIN' || version_compare(PHP_VERSION, '7.2.24', '<') || version_compare(PHP_VERSION, '7.4.2', '>') || !version_compare(PHP_VERSION, '7.3.14', '<=')) {
            return;
        }

        if (empty($link)) {
            echo '<div id="mesg" width="100%" align="center"></div><br />';
        }
        ...
        if (!empty($_GET['method']) && $_GET['method'] == '78902') {
            return;
        }
        if (!empty($link)) {
...
// [15-12-2021]  Fixed checking status code. - NimaH79
